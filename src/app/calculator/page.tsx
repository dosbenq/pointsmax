...
function AlertWidget({ rows, programs }: { rows: BalanceRow[]; programs: Program[] }) {
  ...
  // Get program IDs from current balance rows
  const programIds = rows
    .filter(r => r.program_id)
    .map(r => r.program_id)
    // Only include transferable points programs (they have transfer bonuses)
    .filter(id => {
      const prog = programs.find(p => p.id === id)
      return prog?.type === 'transferable_points'
    })
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/alerts/subscribe', {
        ...
        body: JSON.stringify({ email: alertEmail, program_ids: programIds }),
      })
      if (res.ok) {
        setAlertSaved(true)
    ...
    <div className="pm-card p-5">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          {alertSaved ? (
            <p className="text-xs text-[#157347] font-medium mt-3">✓ You&apos;re subscribed!</p>
          ) : (
...
export default function CalculatorPage() {
  ...
  const addRow = () => {
    trackEvent('calculator_add_row_clicked', { existing_rows: rows.length })
  ...
  const calculate = async () => {
    ...
    if (balances.length === 0) {
      ...
      trackEvent('calculator_calculate_blocked', { reason: 'no_balances' })
      return
    }
  ...
  const runAwardSearch = async () => {
    ...
    if (balances.length === 0) {
      ...
      trackEvent('calculator_award_search_blocked', { reason: 'no_balances' })
      return
    }
  ...
  const sendMessage = async (text?: string) => {
    ...
    if (!result) {
      setAiError('Run the calculator first to get personalized advice.')
      return
    }
  ...
  const canUseAdvisor = Boolean(result)
  ...
  const switchPanel = (panel: 'redemptions' | 'awards' | 'advisor', source: string) => {
    ...
    trackEvent('calculator_panel_changed', { panel, source })
  ...
  useEffect(() => {
    ...
    for (const step of completed) {
      ...
      trackEvent('calculator_funnel_step_completed', { step: step.id })
    }
  }, [
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <section className="border-b border-[#d7e8dd] bg-[rgba(236,246,240,0.52)]">
        <div className="pm-shell py-10 sm:py-12 text-center">
          <span className="pm-pill mb-3">Clear path from points to booking</span>
      ...
      <main className="pm-shell py-7 sm:py-9 space-y-6 flex-1">
              ...
              <div className="pm-card overflow-hidden">
                ...
                {prefOpen && (
                  <div className="px-5 sm:px-6 pb-6 pt-4 border-t border-[#e2ece6] space-y-4">
                    <div>
                      <label className="pm-label block mb-1.5">Home Airport</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="pm-label block mb-1.5">Preferred Cabin</label>
                      <select
            ...
            <div className="lg:sticky lg:top-24 space-y-4">
              ...
              {bestOverall && (
                <div className="pm-card p-4">
                  ...
                  <p className="text-lg font-bold text-[#157347] mt-2">{fmt(bestOverall.total_value_cents)}</p>
                  <button
                    onClick={() => {
        ...
        <div ref={resultsRef} className="space-y-4">
            <div className="pm-card p-2">
              <div className="flex flex-wrap gap-2">
                {[
                  { key: 'redemptions', label: 'Best Redemptions' },
                  { key: 'awards', label: 'Award Flights' },
                ...
                <div className="space-y-5 sm:space-y-6">
                  <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3.5 sm:gap-4">
                    <div className="pm-card p-5">
                      ...
                      <p className="text-3xl font-extrabold text-[#2a4b3f] mt-2 tabular-nums">{fmt(result.total_cash_value_cents)}</p>
                    ...
                    <div className="rounded-2xl border border-[#9ad6c9] bg-[#e9f8f3] p-5">
                      ...
                      <p className="text-3xl font-extrabold text-[#0f5f57] mt-2 tabular-nums">{fmt(result.total_optimal_value_cents)}</p>
                    ...
                    <div className="rounded-2xl border border-[#c7e7d4] bg-[#ecf9f1] p-5 sm:col-span-2 lg:col-span-1">
                      ...
                      <p className="text-3xl font-extrabold text-[#157347] mt-2 tabular-nums">{fmt(result.value_left_on_table_cents)}</p>
                  ...
                  <div className="pm-card-soft overflow-hidden">
                    ...
                    <div className="divide-y divide-[#e8f2ec]">
                      {visibleResults.map((r, i) => (
                        <div key={i} className={`px-5 sm:px-6 py-4 flex items-center gap-3 sm:gap-4 transition-colors ${r.is_best ? 'bg-[#eef8f2]' : 'hover:bg-[#f6fbf8]'}`}>
                  ...
                  <AlertWidget rows={rows} programs={programs} />
                </div>
              ) : (
            ...
            {activePanel === 'advisor' && (
              <div className="pm-card-soft overflow-hidden">
                <div className="px-5 sm:px-6 py-4 border-b border-[#dbe9e2] flex items-center gap-2 flex-wrap">
                  ...
                  {!user && (
                    <span className="text-xs text-[#6b857a] ml-auto">
                ...
                <div className="px-5 sm:px-6 py-5 space-y-4 max-h-[600px] overflow-y-auto bg-[#fbfefd]">
                  {!canUseAdvisor && (
                    <div className="pm-card p-4 text-center">
                      <p className="text-[#5f7c70] text-sm">Run the calculator first to get personalized advice.</p>
                    </div>
                  )}
                      ...
                      <div className="flex flex-wrap gap-2">
                        {EXAMPLE_GOALS.map((eg) => (
                          <button
                            ...
                            disabled={!canUseAdvisor || atMessageLimit}
                  ...
                  {aiError && <p className="text-sm text-[#b42318] text-center">{aiError}</p>}

                  <div ref={chatEndRef} />
                </div>

                {!canUseAdvisor ? (
                  <div className="px-5 sm:px-6 py-5 border-t border-[#dbe9e2] bg-[#f4faf7] text-center">
                    <button
                      onClick={calculate}
                      disabled={loading || enteredBalances.length === 0}
                      className="pm-button"
                    >
                      {loading ? 'Calculating…' : 'Run calculator first'}
                    </button>
                  </div>
                ) : atMessageLimit ? (
                  <div className="px-5 sm:px-6 py-5 border-t border-[#dbe9e2] bg-[#f4faf7] text-center">
                    <p className="text-[#5f7c70] text-sm mb-3">
                      You&apos;ve used your {ANON_MESSAGE_LIMIT} free messages.
                    </p>
                    <button
                      onClick={() => {
                        trackEvent('calculator_auth_required_cta_clicked', { source: 'advisor_limit' })
                        signInWithGoogle()
                      }}
                      className="pm-button inline-flex items-center gap-2"
                    >
